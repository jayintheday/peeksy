...
struct HookSpecTests {
    ...
    private static var snippetURL: URL {
        URL(fileURLWithPath: #filePath)          // Tests/PeeksyCoreTests/HookSpecTests.swift
            .deletingLastPathComponent()          // Tests/PeeksyCoreTests
    ...
    func snippetMatchesSpec() throws {
        ...
        for spec in HookSpec.events {
            ...
            #expect((entry[HookSpec.commandKey] as? String)?.hasSuffix("peeksy-hook.sh") == true)
        }
    }
    ...
    func snippetIsOnlyOurBlock() throws {
        let snippet = HookSpec.snippet(command: "/x/peeksy-hook.sh")
    ...
    func snippetCarriesTheCommand() throws {
        let quoted = HookSpec.shellQuoted("/Users/x/Library/Application Support/Peeksy/peeksy-hook.sh")
    ...
    func snippetMatchesTheCommittedFile() throws {
        ...
        let placeholder = "/PATH/TO/peeksy-hook.sh"
