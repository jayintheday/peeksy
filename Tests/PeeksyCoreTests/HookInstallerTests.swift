...
private struct Scratch: ~Copyable {
    ...
    init(_ name: String) {
        directory = URL(fileURLWithPath: "/tmp")
            .appendingPathComponent("peeksy-tests-\(name)-\(getpid())-\(UInt32.random(in: 0...UInt32.max))")
    ...
    func backups() -> [URL] {
        ...
        return all.filter { $0.lastPathComponent.contains("peeksy-backup-") }.sorted {
...
struct SettingsIOTests {
    ...
    func writesBackup() throws {
        ...
        #expect(backupURL.lastPathComponent.hasPrefix("settings.json.peeksy-backup-"))
    ...
    func backupNaming() {
        ...
        #expect(SettingsIO.backupURL(for: url, now: when).lastPathComponent
                == "settings.json.peeksy-backup-20260727-134501")
    }
}
...
struct HookScriptSyncTests {
    ...
    func creates() throws {
        ...
        let destination = scratch.directory.appendingPathComponent("installed/peeksy-hook.sh")
    ...
    func updatesOnlyWhenBytesDiffer() throws {
        ...
        let source = scratch.directory.appendingPathComponent("bundled.sh")
        let destination = scratch.directory.appendingPathComponent("peeksy-hook.sh")
        scratch.write("#!/bin/sh\nexit 0\n", to: source)
