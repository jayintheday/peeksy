...
struct SessionRowTextTests {
    ...
    private func session(
        id: String,
        cwd: String? = "/Users/v/Desktop/TestRepo/peeksy",
        tty: String? = "ttys003",
        pid: Int32? = 501,
    ...
    func noTitleIsTheOldLayout() {
        ...
        #expect(rows[0].title == "TestRepo/peeksy")
        #expect(rows[0].subtitle == "Bash: npm test")
    }

    /// Both rows, never just the second — showing "peeksy" twice is
    ...
    func duplicateProjectDisambiguatesBoth() {
        ...
        #expect(rows[0].title == "TestRepo/peeksy · ttys001")
        #expect(rows[1].title == "TestRepo/peeksy · ttys002")
    }

    @Test("a lone session in a project gets no tty suffix")
    func singleProjectIsNotDisambiguated() {
        #expect(build([session(id: "a")])[0].title == "TestRepo/peeksy")
    }
    ...
    func ideOwnerIsAppended() {
        ...
        #expect(rows[0].title == "TestRepo/peeksy · Zed")
    }
    ...
    func titleTakesLineOne() {
        ...
        #expect(rows[0].subtitle == "TestRepo/peeksy · Read: orbs-demo.png")
    }
    ...
    func subtitleCarriesEverything() {
        ...
        #expect(rows[0].subtitle == "TestRepo/peeksy · ttys001 · Cursor · Bash: swift test")
    ...
    func theReportedBug() {
        ...
        #expect(rows.allSatisfy { $0.subtitle?.hasPrefix("TestRepo/peeksy · ttys") == true })
    }
    ...
    func permissionOutranksToolSummary() {
        ...
        let rows = build(
            ...
            titles: ["a": "Clean up"])
        #expect(rows[0].subtitle == "TestRepo/peeksy · Bash: rm -rf /tmp/x")
    ...
    func blankTitleIsIgnored() {
        #expect(build([session(id: "a")], titles: ["a": ""])[0].title == "TestRepo/peeksy")
        #expect(build([session(id: "a")], titles: ["a": "   "])[0].title == "TestRepo/peeksy")
    }
