...
struct SocketPathTests {
    ...
    func defaultPath() {
        ...
        #expect(url.path == "/Users/vijay/Library/Application Support/Peeksy/hook.sock")
    }

    @Test("PEEKSY_SOCK overrides everything")
    func envOverride() {
        let url = SocketPath.resolve(env: ["PEEKSY_SOCK": "/tmp/custom.sock"], home: normalHome)
        #expect(url.path == "/tmp/custom.sock")
    }

    @Test("an empty or whitespace override is ignored")
    func emptyOverrideIsIgnored() {
        #expect(SocketPath.resolve(env: ["PEEKSY_SOCK": ""], home: normalHome).path.hasSuffix("hook.sock"))
        #expect(SocketPath.resolve(env: ["PEEKSY_SOCK": "  "], home: normalHome).path.contains("Application Support"))
    }

    @Test("a 90-character home falls back to /tmp rather than overflowing sun_path")
    func longHomeFallsBackToTmp() {
        // 90 chars. Plus "/Library/Application Support/Peeksy/hook.sock" (49)
        ...
        #expect(url.path.hasPrefix("/tmp/peeksy-"))
    ...
    func fallbackIsPerUID() {
        #expect(SocketPath.fallback(uid: 501).path == "/tmp/peeksy-501.sock")
        #expect(SocketPath.fallback(uid: 502).path == "/tmp/peeksy-502.sock")
    }
