...
private func scratch(_ name: String) -> URL {
    URL(fileURLWithPath: "/tmp")
        .appendingPathComponent("agent-notch-capture-\(name)-\(getpid())-\(UInt32.random(in: 0...UInt32.max))")
}
...
struct EventCaptureTests {
    ...
    func offByDefault() {
        ...
        #expect(EventCapture.resolve(arguments: ["AgentNotch"], env: ["AGENT_NOTCH_CAPTURE": ""]) == nil)
        #expect(EventCapture.resolve(arguments: ["AgentNotch"], env: ["AGENT_NOTCH_CAPTURE": "  "]) == nil)
    }
    ...
    func argvWins() throws {
        let capture = try #require(EventCapture.resolve(
            arguments: ["AgentNotch", "--capture", "/tmp/from-argv.jsonl"],
            env: ["AGENT_NOTCH_CAPTURE": "/tmp/from-env.jsonl"]))
    ...
    func envFallback() throws {
        let capture = try #require(EventCapture.resolve(
            arguments: ["AgentNotch"], env: ["AGENT_NOTCH_CAPTURE": "/tmp/from-env.jsonl"]))
    ...
    func captureNeedsAPath() {
        #expect(EventCapture.resolve(arguments: ["AgentNotch", "--capture", "--slice"], env: [:]) == nil)
        #expect(EventCapture.resolve(arguments: ["AgentNotch", "--capture"], env: [:]) == nil)
    }
