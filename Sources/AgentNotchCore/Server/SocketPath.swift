...
public enum SocketPath {
    ...
    /// `AGENT_NOTCH_SOCK` wins (tests and side-by-side runs), else
    /// `~/Library/Application Support/AgentNotch/hook.sock`, else
    /// `/tmp/agent-notch-<uid>.sock` when that would overflow `sun_path`.
    public static func resolve(
        env: [String: String] = ProcessInfo.processInfo.environment,
        home: URL = FileManager.default.homeDirectoryForCurrentUser
    ) -> URL {
        if let override = env["AGENT_NOTCH_SOCK"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !override.isEmpty {
    ...
    public static func fallback(uid: uid_t = getuid()) -> URL {
        URL(fileURLWithPath: "/tmp/agent-notch-\(uid).sock")
    }
