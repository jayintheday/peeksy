...
enum FocusCLI {
    ...
    /// Injected into `TerminalFocuser` so `PeeksyCore` never imports AppKit.
    ...
    private static func focus(tty: String?) -> Int32 {
        guard let tty, !tty.hasPrefix("--") else {
            printErr("usage: Peeksy --focus <tty>   (e.g. ttys003 or /dev/ttys003)")
            return 1
        }
    ...
    private static func doctor() -> Int32 {
        ...
        if let override = ProcessInfo.processInfo.environment["PEEKSY_SOCK"], !override.isEmpty {
            print("                  (from $PEEKSY_SOCK)")
        }
        ...
        if blocked {
            ...
            print(Tcc.remedy)
            print("")
            print("If Peeksy does not appear under Automation, or you have rebuilt the")
            print("app (ad-hoc signing changes the cdhash every build), reset the grant with:")
            print("  tccutil reset AppleEvents com.vijaypatel.peeksy")
        }
    ...
    private static func captureReport(_ arguments: [String]) -> Int32 {
        ...
        guard let url = explicit.map({ URL(fileURLWithPath: ($0 as NSString).expandingTildeInPath) })
            ...
            printErr("usage: Peeksy --capture-report <path>")
            printErr("   or: set $PEEKSY_CAPTURE and omit the path")
            return 2
        }
        ...
        guard let text = try? String(contentsOf: url, encoding: .utf8) else {
            ...
            printErr("  open dist/Peeksy.app --args --capture \(url.path)")
            return 1
        }
    ...
    private static func transcriptTitle(_ arguments: [String]) -> Int32 {
        ...
        guard index + 1 < arguments.count, !arguments[index + 1].hasPrefix("--") else {
            printErr("usage: Peeksy --transcript-title <path to a session .jsonl>")
    ...
    private static func resolvedSocketPath() -> String {
        ...
        return home
            .appendingPathComponent("Library/Application Support/Peeksy/hook.sock")
            .path
    }
    ...
    private static func describeSocket(at path: String) -> String {
        ...
        guard let attributes = try? fm.attributesOfItem(atPath: path) else {
            return "no (nothing at that path — is Peeksy running?)"
        }
