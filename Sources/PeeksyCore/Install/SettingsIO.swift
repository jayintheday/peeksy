...
public enum SettingsIO {
    ...
    /// `settings.json.peeksy-backup-20260727-134501`, in the SAME directory.
    ///
    ...
    public static func backupURL(for url: URL, now: Date, calendar: Calendar = .current) -> URL {
        ...
        return url.deletingLastPathComponent()
            .appendingPathComponent("\(url.lastPathComponent).peeksy-backup-\(stamp)")
    }
    ...
    public static func write(
        ...
        let temp = directory.appendingPathComponent(".\(url.lastPathComponent).peeksy.\(getpid()).tmp")
