...
struct OrbLabView: View {
    ...
    private static let states: [(tint: RowTint, symbol: String, label: String, text: String)] = [
        (.working, "circle.dotted", "agent-notch", "working"),
    ...
    private var controls: some View {
        VStack(alignment: .leading, spacing: 8) {
            heading("Controls")
            HStack(spacing: 16) {
                Toggle("Spin (simulates the `working` gate)", isOn: $spinAll)
                    .toggleStyle(.switch)
                Divider().frame(height: 16)
                Text("Orb size \(Int(orbSide))pt")
                    .font(.system(size: 11).monospacedDigit())
                Slider(value: $orbSide, in: 10...28, step: 1).frame(width: 160)
            }
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(alignment: .bottom, spacing: 20) {
                ForEach(Self.sizes, id: \.self) { size in
                    VStack(spacing: 6) {
                        SessionOrb(tint: .working, spinning: spinAll, side: size)
                        Text("\(Int(size))")
    ...
    private var tintGrid: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(alignment: .top, spacing: 18) {
                ForEach(Self.states, id: \.label) { state in
                    VStack(spacing: 8) {
                        SessionOrb(tint: state.tint, spinning: true, side: orbSide)
                        SessionOrb(tint: state.tint, spinning: false, side: orbSide)
                        Text(state.text)
                            .font(.system(size: 9))
                            .foregroundStyle(.white.opacity(0.4))
                    }
        ...
        VStack(alignment: .leading, spacing: 5) {
            ...
            ForEach(
                [
                    ...
                    "Watch one orb for ~30s: the loop is \(String(format: "%.1f", OrbitsMode.period / OrbProfile.row.speed))s. Any pop at the wrap is a bug.",
...
final class OrbLabWindow {
    ...
    init() {
        ...
        panel.title = "AgentNotch — orb lab"
